...
| Rule | Title | Runtime |
|------|-------|---------|
| CPY001 | Dict ordering assumption — comparing dict view to ordered sequence | cpython |
| CPY002 | Exception.add_note() requires Python 3.11+ | cpython |
| CPY003 | X \| Y union type syntax requires Python 3.10+ | cpython |
| CPY004 | tomllib requires Python 3.11+ | cpython |
| CPY005 | match/case requires Python 3.10+ | cpython |
| CPY006 | asyncio.timeout() / TaskGroup requires Python 3.11+ | cpython |
| CPY007 | Module removed in Python 3.13 | cpython |
| CPY008 | __slots__ may not prevent __dict__ on Python < 3.10 | cpython |
| CPY009 | ExceptionGroup requires Python 3.11+ | cpython |
| CPY010 | @dataclass(slots=True) requires Python 3.10+ | cpython |
| CPY011 | typing.Self requires Python 3.11+ | cpython |
| CPY012 | typing.LiteralString requires Python 3.11+ | cpython |
| CPY013 | typing.override requires Python 3.12+ | cpython |
| CPY014 | typing.TypeAlias requires Python 3.10+ | cpython |
| CPY015 | typing.Never requires Python 3.11+ | cpython |
...
| CPY024 | typing.TypeGuard requires Python 3.10+ | cpython |
| CPY025 | typing.ParamSpec requires Python 3.10+ | cpython |
| CPY026 | typing.io and typing.re removed in Python 3.13 | cpython |
| CPY027 | locale.resetlocale() removed in Python 3.13 | cpython |
| CPY028 | lib2to3 removed in Python 3.13 | cpython |
| CPY029 | locals() semantics changed in Python 3.13 (PEP 667) | cpython |
| CPY030 | sys.path no longer accepts bytes entries in Python 3.11+ | cpython |
| CPY031 | typing.assert_never requires Python 3.11+ | cpython |
| CPY032 | typing.reveal_type requires Python 3.11+ | cpython |
| CPY033 | pathlib.Path.is_relative_to() requires Python 3.9+ | cpython |
| CPY034 | int.bit_count() requires Python 3.10+ | cpython |
| CPY035 | str.removeprefix/removesuffix requires Python 3.9+ | cpython |
| CPY036 | datetime.utcnow() deprecated since Python 3.12 | cpython |
| CPY037 | datetime.utcfromtimestamp() deprecated since Python 3.12 | cpython |
| CPY038 | asyncio.get_event_loop() raises RuntimeError in Python 3.14+ | cpython |
| CPY039 | zoneinfo module requires Python 3.9+ | cpython |
| CPY040 | graphlib module requires Python 3.9+ | cpython |
| CPY041 | dict \| merge operator requires Python 3.9+ | cpython |
| CPY042 | aiter() and anext() builtins require Python 3.10+ | cpython |
| CPY043 | math.lcm() requires Python 3.9+ | cpython |
| CPY044 | math.gcd() with multiple args requires Python 3.9+ | cpython |
| CPY045 | NaN hash behaviour changed in Python 3.10 | cpython |
| CPY046 | open() without encoding= uses platform-dependent encoding before 3.15 | cpython |
| CPY047 | collections.abc.ByteString deprecated, scheduled removal in Python 3.17 | cpython |
...
| CPY057 | pickle default protocol changed to 5 in Python 3.14 | cpython |
| CPY062 | string.templatelib requires Python 3.14+ | cpython |
| CPY063 | annotationlib requires Python 3.14+ | cpython |
| CPY064 | Deprecated AST node types removed in Python 3.14 | cpython |
| CPY065 | pkgutil.find_loader()/get_loader() removed in Python 3.14 | cpython |
| CPY066 | asyncio child watcher classes removed in Python 3.14 | cpython |
| CPY067 | typing.NamedTuple keyword syntax removed in Python 3.15 | cpython |
| CPY068 | typing.no_type_check_decorator deprecated in 3.13, removed in 3.15 | cpython |
| CPY069 | asyncio.iscoroutinefunction() deprecated in Python 3.14 | cpython |
| CPY070 | asyncio event loop policy deprecated in Python 3.14 | cpython |
| CPY071 | pty.master_open()/slave_open() removed in Python 3.14 | cpython |
| CPY072 | importlib.abc resource classes removed in Python 3.14 | cpython |
| CPY073 | sqlite3.version/version_info removed in Python 3.14 | cpython |
| CPY074 | code.__lnotab__ deprecated since Python 3.10 (PEP 626) | cpython |
...
| PPY006 | Monkey-patching built-in types behaves differently on PyPy | pypy |
| PPY007 | sys.intern() identity guarantees differ on PyPy | pypy |
| PPY008 | threading.local() cleanup timing differs on PyPy | pypy |
| PPY009 | id() stability depends on PyPy GC configuration | pypy |
| PPY010 | gc.collect() behaviour differs on PyPy | pypy |
| PPY011 | array.array('u') type code removed in Python 3.13 | both |
| PPY012 | Overriding built-in methods may behave differently on PyPy | pypy |
| PPY013 | sys.getsizeof() raises TypeError on PyPy | pypy |
| PPY014 | String concatenation in loop is O(n²) on PyPy | pypy |
| PPY015 | Generator cleanup timing differs on PyPy | pypy |
| PPY016 | Instance __dict__ order-sensitive access may differ on PyPy | pypy |
| PPY017 | Adding __del__ to existing class not called on PyPy | pypy |
...
| PPY026 | __builtins__ is always a module on PyPy, never a dict | pypy |
| PPY027 | Deleting module/class attributes may be slower on PyPy | pypy |
| PPY028 | readline.parse_and_bind() silently ignored on PyPy | pypy |
| PPY029 | Assigning to __builtins__ has no effect on PyPy | pypy |
| PPY030 | sys.flags values may differ between CPython and PyPy | pypy |
| PPY031 | Integer 'is' identity semantics differ on PyPy | pypy |
...
| PPY045 | sys.settrace() disables JIT and is unreliable on PyPy | pypy |
| PPY047 | ctypes.util.find_library() unreliable on PyPy | pypy |
| PPY049 | GC behavior differs between PyPy and CPython | pypy |
| PPY051 | code.__lnotab__ deprecated on PyPy too | pypy |
| PPY052 | importlib.abc resource classes may differ on PyPy | pypy |
| PPY053 | functools.lru_cache thread safety differs on PyPy | pypy |

...
class UnpackRule(BaseRule):
    ...
    def check(self, node: ast.AST, filename: str) -> list[Finding]:
        ...
        for info in collect_imports(node).imports:
            if info.module == "typing" and info.name == "Unpack":
                findings.append(Finding(
                    file=filename, line=info.line, col=info.col,
                    rule_id=self.rule_id, title=self.title,
                    description="typing.Unpack requires Python 3.11+. Raises ImportError on Python 3.10 and below.",
                    severity=Severity.ERROR, runtime=Runtime.CPYTHON,
                    affected_from="3.0", affected_until="3.10",
                    suggestion="Guard with: if sys.version_info >= (11,): from typing import Unpack -- or use typing_extensions.",

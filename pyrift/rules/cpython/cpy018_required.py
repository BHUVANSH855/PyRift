            ...
            if (
                ...
                findings.append(
                    Finding(
                        ...
                        title=self.title,
                        description=(
                            f"typing.{info.name} requires Python 3.11+ "
                            "(PEP 655). Importing it on Python 3.10 "
                            "or below raises ImportError."
                        ),
                        severity=Severity.ERROR,
                        ...
                        affected_until="3.10",
                        suggestion=(
                            "For Python <3.11 compatibility, use:\n"
                            f"try:\n"
                            f"    from typing import {info.name}\n"
                            "except ImportError:\n"
                            f"    from typing_extensions import {info.name}"
                        ),

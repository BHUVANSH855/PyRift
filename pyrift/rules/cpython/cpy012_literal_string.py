...
class LiteralStringRule(BaseRule):
    ...
    def check(
        ...
        for info in collect_imports(node).imports:
            if info.module == "typing" and info.name == "LiteralString" and not (info.version_guarded and info.version_guarded >= (3, 11)):
                findings.append(Finding(
                    file=filename, line=info.line, col=info.col,
                    rule_id=self.rule_id, title=self.title,
                    description="typing.LiteralString requires Python 3.11+. Raises ImportError on Python 3.10 and below.",
                    severity=Severity.ERROR, runtime=Runtime.CPYTHON,
                    affected_from="3.0", affected_until="3.10",
                    suggestion="Guard with: if sys.version_info >= (3, 11): from typing import LiteralString -- or use typing_extensions.",
                    docs_url="https://peps.python.org/pep-675/",
                ))
        return findings
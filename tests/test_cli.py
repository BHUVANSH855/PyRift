...
class TestCliErrorPaths:
    def test_invalid_python_min(self, tmp_path):
        code, _out, err = run_cli("scan", str(tmp_path), "--python-min", "notaversion")
        assert code != 0
    ...
    def test_json_format(self, tmp_path):
        ...
        code, out, _err = run_cli("scan", str(tmp_path), "--format", "json")
        assert code == 0
        import json
        data = json.loads(out)
    ...
    def test_markdown_format(self, tmp_path):
        py_file = tmp_path / "test.py"
        py_file.write_text("x = 1\n")
        code, out, _err = run_cli("scan", str(tmp_path), "--format", "markdown")
        assert code == 0
        assert out.strip()
    ...
    def test_path_not_found_error(self):
        code, _out, err = run_cli("scan", "C:/nonexistent_pyrift_xyz_12345")
        assert code == 2

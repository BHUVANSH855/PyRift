...
class TestCPY001:
    ...
    def test_detects_items_vs_tuple(self):
        findings = run_rule(self.rule, "assert d.items() == [('a', 1)]")
        assert len(findings) == 1

    def test_clean_vs_set_literal(self):
        # set comparison is order-independent — should NOT flag
        findings = run_rule(self.rule, "assert d.keys() == {'a', 'b'}")
        assert len(findings) == 0

    def test_clean_vs_set_call(self):
        # set() call is safe — should NOT flag
        findings = run_rule(self.rule, "assert set(d.keys()) == {'a'}")
        assert len(findings) == 0

    def test_finding_has_suggestion(self):
        findings = run_rule(self.rule, "d.keys() == ['a']")
        assert len(findings) == 1
